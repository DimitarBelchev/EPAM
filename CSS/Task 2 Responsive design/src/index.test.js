...
const {
    getFirstMediaQueryInner,
} = require("../test-utils/getFirstMediaQueryInner");
...
describe("Responsive Design Intro:", () => {
    ...
    beforeEach(async () => {
        mediaQueryBasicRegExp =
            /@media +screen +and +\( *min-width: *1024px *\)/gi;
        ...
        document.body.insertAdjacentHTML(
            "beforeend",
            `<style>${mobileCssString}</style>`
        );
        document.body.insertAdjacentHTML(
            "beforeend",
            `<style>${styleMediaQueryString}</style>`
        );
    });
    ...
    describe("Viewport meta tag with content:", () => {
        ...
        it("should have two content values", async () => {
            ...
            const element = document.querySelector(viewportSelector);
            const contentAttributeValue = element
                .getAttribute("content")
                .trim();

            const contentValuesSplitted = getContentValueSplitted(
        ...
        it("should have width set to device width", async () => {
            ...
            const hasValue = contentValuesSplitted.some(
                (value) => trimSpaces(value) === "width=device-width"
            );
        ...
        it("should have initial scale set to 1", async () => {
            ...
            const hasValue = contentValuesSplitted.some(
                (value) => trimSpaces(value) === "initial-scale=1"
            );
    ...
    describe("Root custom properties:", () => {
        ...
        beforeEach(() => {
            selectorData = getStyleDeclarationForSelector(
                ":root",
        ...
        it("should have --basic-background-image set", () => {
            const backgroundImageString =
                selectorData["--basic-background-image"];

            const isValidString =
                /linear-gradient\( *to +right, *#2a0845, *#6441A5 *\)/gi.test(
    ...
    it("<body> should have expected styles", async () => {
        ...
        expect(elementStyles).toEqual(expect.objectContaining(expectedStyles));
    });
